...
$2a + 4b + 8c = 186$
 ...
 $a + 2b + 2c = 103$Solving, we find that $(a,b,c) = (13,50, - 5)$.  The number in the square marked by the asterisk is $4a + 3b + 12c = \boxed{142}$
...
A little bit of checking tells us that the units digit must be 2. Now our cube must be in the form of $(10k + 2)^3$; using the \href{https://artofproblemsolving.com/wiki/index.php?title=Binomial\_theorem}{binomial theorem} gives us $1000k^3 + 600k^2 + 120k + 8$. Since we are looking for the tens digit, $\mod{100}$ we get $20k + 8 \equiv 88 \pmod{100}$. This is true if the tens digit is either $4$ or $9$. Casework:
                ...
                \begin{itemize}
                    \item $4$: Then our cube must be in the form of $(100k + 42)^3 \equiv 3(100k)(42)^2 + 42^3 \equiv 200k + 88 \pmod{1000}$. Hence the lowest possible value for the hundreds digit is $4$, and so $442$ is a valid solution.\item $9$: Then our cube is $(100k + 92)^3 \equiv 3(100k)(92)^2 + 92^3 \equiv 200k + 688 \pmod{1000}$. The lowest possible value for the hundreds digit is $1$, and we get $192$. Hence, since $192 < 442$, the answer is $\fbox{192}$
                \end{itemize}
...
We first find the number of vertices on the polyhedron:There are 4 corners per square, 6 corners per hexagon, and 8 corners per octagon. Each vertex is where 3 corners coincide, so we count the corners and divide by 3.  $\text{vertices} = \frac{12 \cdot 4 + 8 \cdot 6 + 6 \cdot 8}{3}=48$.
 ...
 Counting the vertices that are on the same face as an arbitrary vertex, we find that there are 13 vertices that aren't possible endpoints of a line originating from the vertex in the middle of the diagram. You can draw a diagram to count this better: 
                    \begin{figure}[H]
                        \centering
                        \includegraphics[width=0.5\textwidth]{Solution/AIME_I_1988_10_Solution_Figure_2.png}
                    \end{figure}
                     Since 13 aren't possible endpoints, that means that there are 35 possible endpoints per vertex.The total number of segments joining vertices that aren't on the same face is $48\cdot 35\cdot \frac 12 = 24 \cdot 35 = \boxed{840}$
...
Our first choice would be the vertex in the middle (there are $48$ of these), and our second choice would be any of the remaining 47 points minus the 12 that share a face without chosen vertex. Summing these we get 
 \[48 (47 - 12) = 1680\] And we divide by $2$ as before to get $\boxed{840}$
...
Let $A,B,C$ be the weights of the respective vertices. We see that the weights of the feet of the cevians are $A+B,B+C,C+A$. By \href{https://artofproblemsolving.com/wiki/index.php?title=Mass\_points}{mass points}, we have that: 
 \[\dfrac{a}{3}=\dfrac{B+C}{A}\] 
 \[\dfrac{b}{3}=\dfrac{C+A}{B}\] 
 \[\dfrac{c}{3}=\dfrac{A+B}{C}\]
...
\textbf{Solution 4 (Ceva Identity)}
...
A cool identity derived from Ceva's Theorem is that:
 ...
 This problem is quite similar to the 1992 AIME problem 14, which also featured concurrent cevians and is trivialized by the identity used in this solution. For reference, below is a link.\href{https://artofproblemsolving.com/wiki/index.php/1992\_AIME\_Problems/Problem\_14}{https://artofproblemsolving.com/wiki/index.php/1992\_AIME\_Problems/Problem\_14}
