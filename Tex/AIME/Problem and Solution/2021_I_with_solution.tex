...
We look for all positive integers of the form $2^a-2^b<1000,$ where $0\leq b<a.$ Performing casework on $a,$ we can enumerate all possibilities in the table below:
 ...
 We prove this statement as follows:
            ...
            \begin{enumerate} 
        \item The positive integers generated for each value of $a$ are clearly different.
...
This solution refers to the \textbf{Diagram} section.
 ...
 \[\frac{C_1D_1}{CD}=\underbrace{\frac{C_1E}{CE}}_{\substack{\text{right} \\ \triangle C_1CE}}=\underbrace{\frac{D_1E}{DE}}_{\substack{\text{right} \\ \triangle D_1DE}}=\cos\theta. \hspace{14.75mm}(2)\]We apply the Transitive Property to $(1)$ and $(2):$
            ...
            \begin{enumerate} 
        \item We get $\frac{B_1E}{BE}=\frac{C_1E}{CE}=\cos\theta,$ so $\triangle B_1C_1E \sim \triangle BCE$ by SAS, with the ratio of similitude 
...
Let the brackets denote areas. We find $[ABCD]$ in two different ways:
            ...
            \begin{enumerate} 
        \item Note that $\sin(180^\circ-\theta)=\sin\theta$ holds for all $\theta.$ By area addition, we get
...
Define the \textbf{distance} between two frogs as the number of sides between them that do not contain the third frog.
 ...
 We wish to find $E(4,4,4).$ Note that:
            ...
            \begin{enumerate} 
        \item At any moment before the frogs stop jumping, the only possibilities for $(a,b,c)$ are $(4,4,4),(2,4,6),$ and $(2,2,8).$
...
$n$ only needs to satisfy $\sigma(a^n)\equiv 1 \pmod{43}$ and $\sigma(a^n)\equiv 1 \pmod{47}$ for all $a$. Let's work on the first requirement (mod 43) first. All $n$ works for $a=1$. If $a>1$, let $a$'s prime factorization be $a=p_1^{e_1}p_2^{e_2}\cdots p_k^{e_k}$. The following three statements are the same:
            ...
            \begin{enumerate} 
        \item For all $a$, we have $\sigma(a^n)=\prod_{i=1}^k(1+p_i+\cdots+p_i^{ne_i})\equiv1\pmod{43}$.
        ...
        \item For all prime $p$, we have $p+p^2+\cdots+p^n \equiv 0 \pmod{43}$.
            ...
            \begin{enumerate} 
        \item For $p\equiv0\pmod{43}$, no matter what $n$ is, it is true that 
...
We perform casework on $a:$
            ...
            \begin{enumerate} 
        \item $a=1.$ For all positive integers $n,$ we have $\sigma\left(a^n\right)-1=0.$
 
        \item $a$ is a prime number. For all prime numbers $p,$ note that
...
from which $n\equiv0\pmod{43}.$
 ...
 Together, we conclude that $n=\operatorname{lcm}(42,43,46,47)$ is the least such positive integer $n$ for this case.
        ...
        \item $a$ is a composite number. Let $a=\prod_{i=1}^{u}p_i^{e_i}$ be the prime factorization of $a.$ Note that $\sigma(a)$ is a multiplicative function: 
...
Note that $y=x^2-k$ is an upward-opening parabola with the vertex at $(0,-k),$ and $x=2(y-20)^2-k$ is a rightward-opening parabola with the vertex at $(-k,20).$ We consider each condition separately:
            ...
            \begin{enumerate} 
        \item The two parabolas intersect at four distinct points.

 By a quick sketch, we have two subconditions:<ol style="margin-left: 1.5em;" type="A"> 
        \item The point $(-k,20)$ is on or below the parabola $y=x^2-k.$We need $20\leq(-k)^2-k,$ from which $k\geq5.$ 
...
<u>Remark for Subcondition A</u>
 ...
 Recall that if $1\leq k\leq 4,$ then the point $(-k,20)$ is above the parabola $y=x^2-k.$ It follows that for $-k\leq x\leq0:$
            ...
            \begin{itemize}<li>The maximum value of $y$ for the parabola $y=x^2-k$ occurs at $x=-k,$ from which $y=k^2-k\leq12.$</li>
<li>The minimum value of $y$ for the parabola $x=2(y-20)^2-k$ occurs at $x=0,$ from which $y=20-\sqrt{\frac k2}>18.$</li>
        ...
        \item The point $(0,-k)$ is on or below the parabola $x=2(y-20)^2-k.$The lower half of the parabola $x=2(y-20)^2-k$ is $y=20-\sqrt{\frac{x+k}{2}}.$ We need $-k\leq20-\sqrt{\frac k2},$ which holds for all values of $k.$
