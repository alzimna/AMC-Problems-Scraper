...
We see that both sequences have equal numbers of terms, so reformat the sequence to look like: 
 ...
 \begin{align*} Y = \ &12 + 14 + \cdots + 100 + 102\\ X = 10 \ + \ &12 +  14 + \cdots + 100\\ \end{align*}
From here it is obvious that $Y - X = 102 - 10 = \boxed{92 \ \mathbf{(A)}}$.

 \textbf{Note} Another way to see this is to let the sum $12+14+16+...+100=x.$ So, the sequences become
 \begin{align*} X = 10+x \\ Y= x+102 \\ \end{align*}
...
\textbf{Solution 3 (Same Approach as Solution 1)}
...
Instead of solving, we can just categorize and solve.
 ...
 Section 2:This contains B and EFrom skimming we can get we can get answer as maybe B
...
Let $d$ be the length of the trip in miles. Roy used no gasoline for the 40 first miles, then used 0.02 gallons of gasoline per mile on the remaining $d - 40$ miles, for a total of $0.02 (d - 40)$ gallons. Hence, his average mileage was
 \[\frac{d}{0.02 (d - 40)} = 55.\]Multiplying both sides by $0.02 (d - 40)$, we get
 \[d = 55 \cdot 0.02 \cdot (d - 40) = 1.1d - 44.\]Then $0.1d = 44$, so $d = \boxed{440}$. The answer is $(C)$.
...
Since Roy had 0 gasoline usage during the first 40 miles, and we know his usage is 0.02 gallons per mile after that, we can look at all 5 individual answers and simply run them through.
 ...
 If Roy drove (A) 140 miles would mean he spent 140 - 40 = 100 miles on gasoline, so he used 100 * 0.02 = 2 gallons. $140/2$ = 70 miles per gallon which does not equal 55, so (A) is incorrect. Similarly, (B) 240 gives:240 - 40 = 200 miles on gasoline, 200 * 0.02 gallons = 4 gallons, $240/4$ = 60 m/g which does not equal 55.

 Plugging in (C) 440, we get:440 - 40 = 400, 400 * 0.02 = 8, $440/8$ = 55 miles per gallon, so answer choice $\boxed{440 \ \mathbf{(C)}}$ is correct.

 "And we're done."

 
            \begin{itemize}
        \item Richard Rusczyk outro*
            \end{itemize}
...
We can change the insides of the square root into a perfect square and then simplify. 
 ...
 \[= \sqrt{6-6\sqrt{2}+3}+\sqrt{6+6\sqrt{2}+3}\]
 \[= \sqrt{\left(\sqrt{6}-\sqrt{3}\right)^2}+\sqrt{\left(\sqrt{6}+\sqrt{3}\right)^2}\]
...
Square roots remind us of squares. So lets try to make $9 - 6\sqrt{2} = (a-b)^2$. Doing a little experimentation we find that 
 \[9 - 6\sqrt{2} = (\sqrt{6} - \sqrt{3})^2.\] Similarly since $9 + 6\sqrt{2} = (a+b)^2$ we know that 
 \[9 + 6\sqrt{2} = (\sqrt{6} + \sqrt{3})^2.\] 

 We want to find $\sqrt{9-6\sqrt{2}}+\sqrt{9+6\sqrt{2}}$. Using what we found above we know 
 \[\sqrt{9 -6\sqrt{2}}+\sqrt{9+6\sqrt{2}} = (\sqrt{6} - \sqrt{3}) + (\sqrt{6} + \sqrt{3}) = 2\sqrt{6}.\] This is nothing but $\boxed{B) 2\sqrt{6}}$.

 Note: This is basically just Solution 2 except you "do a little experimentation"

 \begin{verbatim}
I'm just saying the fastest would be to say just the answer if you think about it
\end{verbatim}
...
\textbf{Solution 4 (No Words)}
 ...
 \[x=\sqrt{9-6\sqrt{2}}+\sqrt{9+6\sqrt{2}}\]
 \[x^{2}=9-6\sqrt{2}+9+6\sqrt{2}+2\sqrt{\left(9-6\sqrt{2}\right)\left(9+6\sqrt{2}\right)}\]
 \[x^{2}=18+2\sqrt{81-72}\]
 \[x^{2}=18+2\sqrt{9}\]
 \[x^{2}=18+6\]
 \[x^{2}=24\]
 \[x=\pm\sqrt{24}\]
 \[x=\pm2\sqrt{6}\]
 \[\boxed{\textbf{(B) } 2\sqrt{6}}\]




\textbf{Solution 5 (Bash / Int. Alg.)}
...
\textbf{Solution 1}
                    ...
                    \begin{figure}[H]
                        \centering
                        \includegraphics[width=0.5\textwidth]{Solution/AMC_10_10A_2011_18_Solution_Figure_1.png}
                    \end{figure}
...
Since all the answer choices are around $50\%$, we know the town's starting population must be around $600$. We list perfect squares from $400$ to $1000$.
 \[441, 484, 529,576,625,676,729,784,841,900,961\]We see that $484$ and $784$ differ by $300$, and we can confirm that $484$ is the correct starting number by noting that $484+150=634=25^2+9$. Thus, the answer is $300/484\approx \boxed{\textbf{(E) } 62\%}$.

 
(minor edit by mrmatthewzhang)
...
Ideally, if $A$ and $B$ had chords $c_1, c_2$, the only way $c_1 \cap c_2 \neq \emptyset$ is if $AB \leq r$. Thus, the maximum possible intersections occur when the ends of the chords $C$ and $D$ degenerate, forming equilateral triangle $ABC$ with radius $r$.

 Thus, if $O$ is the center of this circle, then $B$ must lie on the $\frac{2}{3}$ arc of the circumference such that $AB \leq r$. If it were to lie on the other $\frac{1}{3}$, then $AB > r$ which is not allowed.

 However, we limit $B$ to be counterclockwise to that of $A$, so half of the viable $\frac{2}{3}$ arc region is invalid (as that implies $B$ must be on a chord that is formed by moving counterclockwise). Thus, the probability is $\boxed{\frac{1}{3}}$.
...
WLOG, draw $ABCDE$ such that point $A$ is at the top, and write the letters in counterclockwise order. WLOG, fill in $A$ first. There are $6$ ways to do so. From here we proceed with casework on the color of $B$:
            ...
            \begin{itemize}
        \item Case 1: $B$ is the same color as $A$
            \end{itemize}
            \begin{verbatim}
...
\end{verbatim}
            ...
            \begin{itemize}
        \item Case 2: $B$ and $A$ are different colors
            \end{itemize}
            \begin{verbatim}
...
\end{verbatim}
            ...
            \begin{itemize}
        \item Case 2.1: $C$ and $B$ are the same color
            \end{itemize}
            \begin{verbatim}
...
\end{verbatim}
            ...
            \begin{itemize}
        \item Case 2.2: $C$ and $B$ are different colors
            \end{itemize}
            \begin{verbatim}
...
Notice that a minimum of, $3$ and a maximum of $5$ colours can be used.
            ...
            \begin{itemize}
        \item Case $1$: $5$ colours used.
            \end{itemize}
            This is straight forward and there are $6P5 = 720$ ways.

 
            \begin{itemize}
        \item Case $2$: $4$ colours used.
            \end{itemize}
...
\textbf{Solution 9 (Elimination of answers)}
...
After Alice says all her numbers, the numbers not mentioned yet are 
 \[\text{Alice: } 2,5,8,11,14,17,\cdots,998.\]After Barbara says all her numbers, the numbers that haven't been said yet are 
 \[\text{Barbara: } 5,14,23,32,41,50,\cdots,995.\]After Candice, the list is 
 \[\text{Candice: } 14,41,68,\cdots,986.\]Notice how each list is an arithmetic sequence where the common differene is thrice the common ratio of the previous list and the first term is the second term of the previous list. Now that the pattern is clear, we construct the rest of the lists: 
 \[\text{Debbie: } 41,122,203,\cdots,959\]
 \[\text{Eliza: } 122,365,608,878\]
 \[\text{Fatima: } 365\]
...
Notice that Alice has skipped the numbers $3n-1$ for $n=1,2,3,...,333$.Namely, 
 \[3\cdot1-1,3\cdot2-1,3\cdot3-1,...,3\cdot333-1\]Thus the numbers that Barbara skips are 
 \[3\cdot2-1,3\cdot5-1,3\cdot8-1,...\]or in a more general expression, $3(3n-1)-1$ for $n=1,2,3,...$.Namely, 
 \[3\cdot(3\cdot1-1)-1,3\cdot(3\cdot2-1)-1,3\cdot(3\cdot3-1)-1,...\]Repeating the pattern until George, we have the first number he says,
 \[3(3(3(3(3(3\cdot1-1)-1)-1)-1)-1)-1= \boxed{\textbf{(C) } 365}\]In addition, note that the second number George says exceeds $1000$.
...
Similar to Solution 1 we find that the only number that George can say must leave a remainder of $2$ when divided by $3$, and that it must also leave a remainder of $5$ when divided by $9$. Since we as human beings are usually lazy (we mean smart), and that MAA provides answer choices, we check all the possible numbers and find that our answer is $\boxed{\textbf{(C) } 365}$.
...
\textbf{Solution 7 (backwards solution 3)}
...
\textbf{Solution 1}
                    ...
                    \begin{figure}[H]
                        \centering
                        \includegraphics[width=0.5\textwidth]{Solution/AMC_10_10A_2011_24_Solution_Figure_1.gif}
                    \end{figure}
